...
import com.cheyifu.icar.tabHome.softKey.SoftKey.KeyType;
...
//	private SoftKey [] punctKeys;
	...
	public SoftKey[] initSoftKeys() {
		...
		SoftKey [] result=new SoftKey[list.size()];
//		char cha='a';
		for(int index=0;index<result.length;index++){
			SoftKey sofkey=new SoftKey();
			sofkey.setText(x[index]);
			result[index]=sofkey;
		}
//		punctKeys=initPunctKeys();
	...
	public SoftKey[] measureSoftKeysPos(SoftKey[] softKeys) {
		...
		for(int index=OneCol;index<OneCol+OneCol;index++){
			softKeys[index].setX(blockWidth/2+(index%OneCol)*blockWidth);
		...
		//第二行9个字符按钮
		for(int index=OneCol+OneCol;index<OneCol+twoCol+OneCol;index++){
			softKeys[index].setX((index-(OneCol+OneCol-1))*blockWidth);
		...
		//第三行7个字符按钮
		for(int index=OneCol+twoCol+OneCol;index<softKeys.length;index++){
			softKeys[index].setX((index-(OneCol+twoCol+OneCol-1)+1)*blockWidth);
			softKeys[index].setY(blockHeight*7/2);
			softKeys[index].setWidth(blockWidth);
			softKeys[index].setHeight(blockHeight);
		}
...
//			confirmBtn.setX(getWidth()-blockWidth*5/4);
			confirmBtn.setX(blockWidth*3/4);
...
//		canvas.drawLine(0, blockHeight*(row-3), getWidth(),blockHeight*(row-3), keyBorderPaint);
		...
		for(int index=0;index<OneCol+OneCol;index++){
			int x=(index+1)*blockWidth;
			canvas.drawLine(x, blockHeight, x, blockHeight*2, keyBorderPaint);
		}
		
		for(int index=0;index<=twoCol+OneCol+OneCol;index++){
			int x=index*blockWidth+blockWidth/2;
			canvas.drawLine(x, blockHeight*2, x, blockHeight*3, keyBorderPaint);
		}
		
		for(int index=0;index<=softKeys.length-twoCol-OneCol-OneCol;index++){
			int x=(index+1)*blockWidth+blockWidth*3/2;
			canvas.drawLine(x, blockHeight*3, x, blockHeight*4, keyBorderPaint);
		}
		
//		for(int index=0;index<5;index++){
//			int x=(index+1)*blockWidth*3/2;
//			canvas.drawLine(x, blockHeight*4, x, blockHeight*5, keyBorderPaint);
//		}
